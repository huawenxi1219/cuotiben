...
选择题必须提供options字段（4个选项）。只返回JSON数组。"""
            ...
            def build_error_list_page(subject):
                ...
                if not subject_items:
                    list_view.controls.append(ft.Container(content=ft.Text("  暂无错题", size=16, color=ft.Colors.GREY_600),
                                                           padding=20))
                else:
                    for item in subject_items:
                        try:
                            ...
                            list_view.controls.append(ft.Container(
                                content=ft.Column([
                                    ft.Row([
                                        ft.Text(f"❌ {ts}", size=13, color=ft.Colors.GREY_600),
                                    ], alignment=ft.MainAxisAlignment.SPACE_BETWEEN),
                                    ft.Text(preview, size=14, max_lines=3),
                                ], spacing=4),
                                padding=12, border_radius=12, bgcolor="#FFEBEE",
