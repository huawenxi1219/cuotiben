...
def main(page: ft.Page):
    ...
    if os.path.exists(old_data_dir) and os.path.isdir(old_data_dir):
        # 检查新目录是否为空（避免重复迁移）
        is_empty = True
        for f in ["errors.jsonl", "vocabulary.jsonl", "notes.jsonl"]:
            if os.path.exists(os.path.join(DATA_DIR, f)):
                is_empty = False
                break
        if is_empty:
            try:
                print("[迁移] 开始从旧公共目录迁移数据...")
                ...
                print("[迁移] 数据复制完成，开始修正路径...")
                # 修正 JSONL 内的绝对路径为相对路径
